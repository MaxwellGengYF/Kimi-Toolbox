"""kimi_cli.native_loader — resolve and load the ``kimix_native`` shim.
...
using an ordered list of search paths, inserts the first usable directory on
``sys.path`` and imports it once (cached). When the binaries are missing the
loader degrades to the pure-Python fallback — no import-time side effects,
never raises (except for the documented ``KIMIX_NATIVE=1`` contract).

This module is the single home of the loader logic. It previously lived in
``kimix.native_loader`` and was delegated to from here by loading that file
STANDALONE by file path (because importing the ``kimix`` package executes
``kimix/__init__.py`` which, via ``kimix.utils._globals -> kimi_agent_sdk ->
kimi_cli.app -> kimi_cli.soul.agent``, re-enters ``kimi_cli`` mid-import).
The logic now lives here directly and — since it only uses the standard
library — never imports ``kimix``, so that cycle is gone by construction.
``kimix`` consumers import this module directly.
...
2. **default**: ``<repo root>\\bin`` — the current project work-dir where
   ...
   "Repo root" is the kimix workspace root — the first ancestor of this file
   that contains the ``bin/kimix_native`` shim (or the root
   ``KIMIX_NATIVE_VERSION`` marker), falling back to ``os.getcwd()`` when the
   repo layout is not detectable, e.g. an installed wheel.
...
def _repo_root() -> str:
    """Kimix workspace root whose ``bin/`` holds the staged native extension.

    The loader lives at ``<root>/kimi-cli/src/kimi_cli/native_loader.py``
    inside the uv workspace, while the staged binaries (and the tracked
    ``kimix_native`` shim) live at ``<root>/bin``.  The root is therefore the
    first ancestor of this file that contains the ``bin/kimix_native`` shim
    (or the root ``KIMIX_NATIVE_VERSION`` marker, which ``_fallback_version``
    reads).  Falls back to ``os.getcwd()`` when the repo layout is not
    detectable (e.g. an installed wheel).
    """
    here = os.path.dirname(os.path.abspath(__file__))  # .../kimi_cli
    current = here
    while True:
        if os.path.isdir(os.path.join(current, "bin", "kimix_native")):
            return current
        if os.path.isfile(os.path.join(current, "KIMIX_NATIVE_VERSION")):
            return current
...
def _fallback_version() -> str:
    ...
    try:
        version_file = os.path.join(_repo_root(), "KIMIX_NATIVE_VERSION")
...
def __getattr__(name: str):
    ...
    if module is None:
        raise AttributeError(
            f"kimi_cli.native_loader has no attribute {name!r} "

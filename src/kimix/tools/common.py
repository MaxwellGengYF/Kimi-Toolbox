...
# Long commands chaining multiple logical operators are hard to debug and
# error-prone as shell one-liners; the LLM backend is nudged towards the
# Python tool.  The warning fires only for genuinely complex one-liners:
# longer than 100 chars AND at least four logical operators.
_PIPE_ADVICE_MIN_LENGTH = 100
_PIPE_ADVICE_MIN_OPERATORS = 4

# PowerShell logical word operators (matched case-insensitively as whole
# tokens).  Bash and PowerShell share the symbolic operators ``&&`` / ``||`` /
# ``|`` / ``!``.
_PWSH_LOGICAL_WORD_OPS = ("-and", "-or", "-xor", "-not")


def _long_command_warning(length: int, operators: int) -> str:
    """Build the concise append-ready warning message."""
    return (
        f"\n[WARNING] Long command ({length} chars, {operators} operators); "
        "prefer the Python tool for multi-step logic."
    )


def _match_pwsh_word_op(cmd: str, i: int, n: int) -> int:
    """Return the length of a PowerShell logical word operator at ``i``, or 0.

    Matches ``-and`` / ``-or`` / ``-xor`` / ``-not`` (case-insensitive) as a
    whole token delimited by identifier boundaries, so ``-android``,
    ``-notepad`` or ``foo-and`` are never misdetected as operators.
    """
    if cmd[i] != "-" or i + 1 >= n:
        return 0
    if i > 0 and (cmd[i - 1].isalnum() or cmd[i - 1] in "-_"):
        return 0
    for token in _PWSH_LOGICAL_WORD_OPS:
        token_len = len(token)
        if cmd[i : i + token_len].lower() == token:
            j = i + token_len
            if j >= n or not (cmd[j].isalnum() or cmd[j] == "_"):
                return token_len
    return 0


def _long_pipeline_advice(cmd: str) -> str:
    """Return a concise warning when *cmd* is long and chains multiple logical operators.

    Triggers only when the command is longer than 100 characters AND contains at
    least four *real* logical operators outside quotes, comments and command
    substitutions: ``&&``, ``||``, ``|`` (pipeline, incl. ``|&``), ``!``
    (negation), and the PowerShell word operators ``-and`` / ``-or`` / ``-xor``
    / ``-not``.  The result starts with ``\n[WARNING]`` so it can be appended
    directly to a tool return message; returns ``""`` otherwise.

    Detection is deliberately cheap (the common case runs in C speed):
      1. ``len(cmd)`` is O(1) and rejects short commands immediately.
      2. C-level ``str.count`` passes over the operator character classes
         (``|``, ``&``, ``!``, ``-``) reject commands with fewer than four
         candidate characters — every logical operator contains at least one.
      3. Only then a quote-aware scan verifies the operators are real, skipping
         ``'...'`` / ``"..."`` / ``$'...'`` quoted regions, ``$(...)`` /
         backtick command substitutions, ``#`` comments, ``!=`` comparisons,
         and word operators glued to identifiers (``-android``, ``foo-and``) —
         with early exit as soon as the fourth operator is found, so the full
         string is walked only when needed.
    """
    if len(cmd) <= _PIPE_ADVICE_MIN_LENGTH:
        return ""
    if sum(cmd.count(ch) for ch in "|&!-") < _PIPE_ADVICE_MIN_OPERATORS:
        return ""
    operators = 0
    i = 0
    n = len(cmd)
    while i < n:
        ...
        if c == "'":
            ...
            i = n if end == -1 else end
        elif c == "#" and (i == 0 or cmd[i - 1].isspace() or cmd[i - 1] in ";&|({"):
            # ``#`` at the start of a word begins a comment (bash & PowerShell);
            # this also hides ``#!`` shebangs and operators inside comments.
            end = cmd.find("\n", i + 1)
            i = n if end == -1 else end + 1
        elif c == "&" and i + 1 < n and cmd[i + 1] == "&":
            operators += 1  # `&&` logical AND
            if operators >= _PIPE_ADVICE_MIN_OPERATORS:
                return _long_command_warning(len(cmd), operators)
            i += 2
        elif c == "|":
            # ``||`` (logical OR) and ``|`` / ``|&`` (pipeline) all count.
            i += 2 if i + 1 < n and cmd[i + 1] == "|" else 1
            operators += 1
            if operators >= _PIPE_ADVICE_MIN_OPERATORS:
                return _long_command_warning(len(cmd), operators)
        elif c == "!" and (i + 1 >= n or cmd[i + 1] != "="):
            # ``!`` negation (PowerShell ``!``, bash ``[[ ! ... ]]``) — but
            # never ``!=`` comparisons nor ``hello!`` glued to a word.
            ...
            token_len = _match_pwsh_word_op(cmd, i, n)
            if token_len:
                operators += 1
                if operators >= _PIPE_ADVICE_MIN_OPERATORS:
                    return _long_command_warning(len(cmd), operators)

...
- **验证门**：存在未完成 todo、todo 验证失败、或修改了文件却没运行任何检查时，回合不允许结束，失败信息会反馈给 Agent 继续修复。
...
- **上下文计量**：上下文用量明显变化时，提醒 Agent 用 `Memory` 工具持久化关键事实。
- **决策感知压缩**：压缩摘要保留 `Decisions & Conclusions` 与 `Verification Status` 两节，早期决策与已验证的工作不丢失。
- **上下文剪枝**：自动清理过期工具输出、thinking 块与近似重复内容，回收上下文空间。

### 可执行验证的 TodoList

`TodoList` 工具不仅跟踪多步计划，还能*证明*计划真的完成：

- 任意 todo 可附带验证 `code`：内联 Python、`.py` 文件、`!` 前缀的 Shell 命令（如 `!pytest tests/ -x -q`）或 `.sh`/`.ps1` 文件。
- 标记 todo 为 `done` 时自动执行验证——失败会以错误返回，并通过验证门阻止回合结束。
